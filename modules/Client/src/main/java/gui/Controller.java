...
public class Controller {
            ...
            ObservableList<PieChart.Data> pieChartData =
                    FXCollections.observableArrayList(
                            ...
                            new PieChart.Data("Public Transport",
                                    Double.parseDouble(info.get("savedPublicTransport")
                                            .toString())),
                            new PieChart.Data("Home Temperature",
                                    Double.parseDouble(info.get("savedHeatConsumption")
                                            .toString())),
                            new PieChart.Data("Bike",
                                    Double.parseDouble(info.get("bikeSaved").toString())),
                            new PieChart.Data("Local Product",
                                    Double.parseDouble(info.get("localSaved")
                                            .toString())),
                            new PieChart.Data("Solar Panel",
                                    Double.parseDouble(info.get("savedSolar")
                                            .toString()))
                    );
    ...
    private void loadAchievements(int page, Stage appStage) {
        ...
        for (int i = start; i <= end; i++) {
            char co = bits.charAt(i);
            boolean cond = co == '0';
            if (cond) {
                String bo = "ach" + i;

                Pane pane = (Pane) scene.lookup("#" + bo);
                pane.setOpacity(0.2);
            }
        }
    }
    ...
    protected void handleLogoutButtonAction(ActionEvent event) throws IOException {
        ...
        Parent root = FXMLLoader.load(getClass()
                .getClassLoader().getResource("fxml/mainLogin.fxml"));
    ...
    private void handleAddVeganMealButtonAction(ActionEvent event) throws IOException {
        ...
        if (!cc.checkToken()) {
            ...
            cc.postMeal(Double
                    .parseDouble(amountVegetarianMeal
                            .getText()),
    ...
    private void handleAddFriendButtonAction(ActionEvent event) throws IOException {
        ...
        //                //            new VeganMeal(ClientBuilder
        //                .newClient()).sendVeganMeal(portions);
        //            } catch (NumberFormatException e) {
        //                AlertHelper
        //                        .showAlert(Alert.AlertType.ERROR, owner, "Wrong input type!",
    ...
    private void handleAddLocalProductButtonAction(ActionEvent event) throws IOException {
        ...
        if (!cc.checkToken()) {
            ...
            cc.postLocalProduce(Double
                            .parseDouble(amountLocalProduct.getText()),
